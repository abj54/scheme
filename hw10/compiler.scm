...
(define (compile exp target linkage)
  (cond ((self-evaluating? exp)
        ...
        ((map? exp)
         (compile-map exp target linkage))
...
(define (compile-apply exp target linkage)
  (let ((proc-code (compile (apply-proc exp) 'proc 'next))
        (operand-codes
         (compile (apply-parlist exp) 'argl 'next)))
    ;(display operand-codes)
    ;(newline)
    (preserving '(env continue)
                proc-code
                (preserving '(proc continue)
...
;       (operand-codes (compile (map-parlist exp) 'argl 'next)))
;    (preserving '(env continue)
;                proc-code
;                (preserving '(continue proc argl)
;                            operand-codes
;                            iterate))))
