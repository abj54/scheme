        ...
        (define (execute)
          (let ((insts (get-contents pc)))
            (if (null? insts)
                'done
                (begin
                  (let ((bp (get breakpoints (car insts))))
                    (cond ((null? bp)
                           (go-execute))
                          (else
                           (let ((bpoint (car bp)))
                             (display "HIT BREAKPOINT: LABEL ")
                             (display (breakpoint-label bpoint))
                             (display "  & LINE ")
                              (display (breakpoint-offset bpoint))
                               (newline)))))))))
        ...
        (define (dispatch message)
          (cond ((eq? message 'start)
                ...
                ((eq? message 'set-breakpoint!)
                 (lambda (label offset)
                   (let ((bp (make-breakpoint label offset labels)))
                     (if (not (exist breakpoints bp))
                         (begin
                         ...
                         (display offset)
                         (newline))
                         (display "Breakpoint already exist")))))
                
                ((eq? message 'cancel-breakpoint!)
                 (lambda (label offset)
                   (let ((bp (make-breakpoint label offset labels)))
                     (if (exist breakpoints bp)
                         (begin
                           (set! breakpoints (remove breakpoints bp))
                           (display "Breakpoint removed"))
                         (display "no breakpoint to remove")))))
                           
                   
                ((eq? message 'cancel-all-breakpoints!)
                 (display "Removed all breakpoints")
                 (set! breakpoints '()))

                ((eq? message 'proceed)
                 (go-execute))

                ((eq? message 'putlabels)
...
(define (remove breakpoints bp)
  (let ((real (breakpoint-real bp)))
    (cond ((null? breakpoints) '())
          ((= real (breakpoint-real (car breakpoints)))
           (remove (cdr breakpoints) bp))
          (else
           (cons (car breakpoints) (remove (cdr breakpoints) bp))))))  
;;;
(define (get breakpoints bp)
  (let ((real (breakpoint-real bp)))
    (cond ((null? breakpoints) '())
          ((= (+ real 1) (breakpoint-real (car breakpoints)))
           (list (car breakpoints)))
          (else
           (get (cdr breakpoints) bp)))))

(define (breakpoint-label bpoint)
  (car bpoint))
(define (breakpoint-offset bpoint)
  (cadr bpoint))
(define (breakpoint-real bpoint)
  (caddr bpoint))


(define (make-breakpoint label offset labels)
  (let ((lab (assoc label labels)))
    (if lab
        (list label offset (+ offset (label-count lab))))))



(define (exist breakpoints bp)
  (let ((real (breakpoint-real bp)))
    (cond ((null? breakpoints) #f)
          ((= real (breakpoint-real (car breakpoints)))
           #t)
          (else
           (exist (cdr breakpoints) bp)))))
...
(define (extract-labels text receive)
  (define (with-count num text receive )
     (if (null? text)
      (receive '() '()) 
      (with-count
       (if (symbol? (car text))
                  ...
                  (if (assoc next-inst labels)
                      ...
                      (receive insts
                               ...
                               (cons (make-label-entry next-inst
                                                       insts num)
                                     labels))))
               (else (receive (cons (make-instruction next-inst num)
                                    insts) labels))))))))
  (with-count 0 text receive))
...
(define (make-label-entry label-name insts num)
  (list label-name insts num))
...
(define (label-inst exp)
  (cadr exp))
(define (label-count exp)
  (caddr exp))
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;;;
...
(define (make-elementary-exp exp machine labels)
  (cond ((constant-exp? exp)
         (let ((c (constant-exp-value exp)))
           (lambda () c)))
        ((AND (label-exp? exp) (pair? (cadr exp)))
         (lambda () (lookup-label
                     labels
                     (get-contents (get-register
                                    machine
                                    (register-exp-reg
                                     (cadr exp)))))))
