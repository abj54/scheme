      ...
      (if (pair? (car args))
          ...
          (cond ((delayed? (car args)) (cons (make-thunk (first-operand exps) env)
                                             (list-of-values-new
                                              ...
                                              (rest-operands exps)
                                              env)))
                ((dynamic? (car args)) (cons (do-dynamic (first-operand exps)
                ...
                ((reference? (car args)) (cons
                                          (make-reference (first-operand exps) env)
                                          (list-of-values-new
                                           (rest-arg args)
                                           (rest-operands exps)
                                           env))))
           (cons (xeval (first-operand exps) env)
                (list-of-values-new (rest-arg args) (rest-operands exps) env)))))
...
(define (s450)
  ...
  (let ((input (read)))
    ;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
    ;;;SPECIAL-FORM IS THE INPUT;;;;;;;;;;;;;;;;;;;
    ;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
    ;;(cond ((AND( lookup input special-table) (symbol? input))
      ;;     (display "Special Form: ") (display  input))
        ;;  (else
